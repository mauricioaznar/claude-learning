# s3-presigned-post-uploads · nestjs + svelte

Presigned-POST uploads to S3-compatible storage. The server only signs and
records; the file goes **browser → storage directly**. The UI is a **Svelte**
...
cp .env.example .env
npm install
npm run start:dev             # API on http://localhost:3002
```

(This is TypeScript: `npm run build` then `npm run start:prod` runs `dist/`.)

**Svelte client** (separate Vite app):

```sh
cd client
npm install
npm run dev                   # dev UI on http://localhost:5173, proxies API to :3002
```
...
curl -s -X POST localhost:3002/uploads -H 'content-type: application/json' \
  -d '{"filename":"doc.pdf","contentType":"application/pdf","size":50000}'

curl -s -o /dev/null -w '%{http_code}\n' -X POST localhost:3002/uploads \
...
| `POST` | `/uploads` | validate (DTO + service policy) + return a presigned POST |
