...
(`setTimeout`). It's the same substrate the `nestjs-lab` batch loader stands on;
...
Run it: `npm --prefix async-lab install` once, then
`npm --prefix async-lab run dev` → http://localhost:5181
